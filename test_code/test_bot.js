...
// Объект для отслеживания состояния чата (в админском режиме или нет)
let adminMode = {};

bot.onText(/\/start/, (msg) => { // функция : Если админ - то вылазит сообщение вместо кнопок
	const chatId = msg.chat.id;
	const userId = msg.from.id.toString();

	// Проверяем, является ли пользователь администратором
	if (userId === adminChatId) {
		// Если пользователь - администратор, устанавливаем режим админа для чата
		adminMode[chatId] = true;
		// Отправляем сообщение администратору
		bot.sendMessage(chatId, 'Вы являетесь администратором данного чат-бота. Вам недоступны команды пользователей 😉');
	} else {
		// Если пользователь не является администратором, устанавливаем режим пользователя
...
bot.on('message', (msg) => {
	...
	if (msg.text === 'Связаться с нами 📱') {
		...
		bot.sendMessage(chatId, 'Наш оператор ответит на ваше сообщение в ближайшее время. Пожалуйста, напишите ваш вопрос. Команды пользователя вам недоступны.', {
			reply_markup: {
				keyboard: [[{ text: 'Покинуть чат' }]],
				resize_keyboard: true,
...
bot.on('callback_query', (callbackQuery) => {
	...
	} else if (forwardingSessions[adminChatId] && forwardingSessions[adminChatId] !== adminId) {
		const userChatId = forwardingSessions[adminChatId];
		bot.sendMessage(userChatId, `Ответ от администратора:\n${callbackQuery.message.text}`);
	}
});
